...
import org.hyzionstudios.hyextras.action.*;
import org.hyzionstudios.hyextras.advanced.ClearPlayerOverridesAction;
import org.hyzionstudios.hyextras.advanced.PlayerHideEntityAction;
import org.hyzionstudios.hyextras.advanced.PlayerShowEntityAction;
import org.hyzionstudios.hyextras.condition.CooldownReadyCondition;
import org.hyzionstudios.hyextras.condition.HasTagCondition;
import org.hyzionstudios.hyextras.condition.InteractionTypeCondition;
import org.hyzionstudios.hyextras.condition.IsOperatorCondition;
import org.hyzionstudios.hyextras.condition.PlayerHiddenCondition;
import org.hyzionstudios.hyextras.condition.VariableCondition;
import org.hyzionstudios.hyextras.condition.VolumeHasTagCondition;
import org.hyzionstudios.hyextras.condition.WorldTimeBetweenCondition;
import java.util.List;
import java.util.logging.Level;

/**
 * Registers all HyExtras effects and conditions with the native Trigger Volume system.
    ...
    public static synchronized void register(HyExtrasPlugin plugin) {
        ...
        if (!ok) {
            plugin.getLogger().at(Level.WARNING)
                    .log("HyExtras: trigger volume type registration did not fully complete; will retry later.");
            return;
        }

        registered = true;
        plugin.getLogger().at(Level.INFO)
                .log("HyExtras: registered 22 effects and 8 conditions.");
    }
}
