 ...
 *
 * <ul>
 *   <li>{@code Title} / {@code Subtitle} — sent via PacketAPI title support</li>
 *   <li>{@code ActionBar} — sent via PacketAPI notification support</li>
 * </ul>
 *
 * <p>Supports {@link StringTemplate} placeholders and HyExtras color/style codes.
 *
 * <p>JSON config:
 * <pre>{@code
 * {
    ...
    public void execute(TriggerContext ctx) {
        ...
        try {
            ...
            if (hasTitle) {
                ...
                String resolvedSubtitle = null;
                if (subtitle != null && !subtitle.isBlank()) {
                    resolvedSubtitle = StringTemplate.resolve(subtitle, ctx, vars);
                }
                HyExtrasPlugin.get().getPacketApi().sendTitle(
                        pr.getUuid(),
                        resolvedTitle,
                        resolvedSubtitle,
            ...
            if (hasBar) {
                ...
                HyExtrasPlugin.get().getPacketApi().sendActionBar(pr.getUuid(), resolvedBar);
            }
        } catch (Exception e) {
            HyExtrasPlugin.get().getLogger()
