...
[`cancel_interaction`](effects#cancel_interaction) reliably blocks the interaction in time.
...
3. To deny the interaction, add [`cancel_interaction`](effects#cancel_interaction) to the effects
   (or rejection effects). To shove the player back, add
   [`push_back_player`](effects#push_back_player).

Filter by interaction kind with the [`interaction_type`](conditions#interaction_type) condition:
...
The `tagValue` is the native `InteractionType` name. The [`interaction_type`](conditions#interaction_type)
...
| Effect | Behavior |
|---|---|
| [`block_volume_interactions`](effects#block_volume_interactions) | Adds a volume to the blocked set. Players inside it can't interact with anything. |
| [`allow_volume_interactions`](effects#allow_volume_interactions) | Adds a sub-volume to the allowed-override set; players inside it bypass the block. |
...
[`set_volume_interactable`](effects#set_volume_interactable) and
[`clear_volume_interactable`](effects#clear_volume_interactable) override the static tags until
...
- [Effects](effects#interaction) — interaction effects.
- [`interaction_type` condition](conditions#interaction_type).
- [Internals: TriggerExtras](internals-trigger-extras) — the bridge implementation.
