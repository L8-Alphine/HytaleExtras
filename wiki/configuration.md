...
|---|---|---|---|
| `advancedPacketActions` | boolean | `true` | Master switch for per-player packet features: `player_hide/show_entity` packets, titles, action bars, camera packets, and the API packet helpers. See [Visibility & Packets](visibility-and-packets). |
| `entityPacketFiltering` | boolean | `false` | **Experimental.** Filters outbound `EntityUpdates` for best-effort non-player entity hiding. Runs on the packet path and can affect join timing — leave off unless testing. Required for visual [TagNPC](tag-npc) hiding. |
...
| `playerVisibilityPolicySync` | boolean | `true` | Periodically applies `IsStoryArea`/`GroupArea` [volume visibility policy](visibility-and-packets#volume-visibility-policy-tags) to player-to-player packets so volume tags work without a custom effect. |
...
Knobs added in the fine-tuning pass. See [Fine-Tuning](fine-tuning) for the behavior behind each.
...
Each of the six [modules](modules) has three keys following the pattern
`modules.<id>.<field>`:
...
Controls [placeholder resolution](string-templates).
...
Provider-scoped image assets. See [Image Icons](image-icons).
...
Runtime entity state. See [TagNPC](tag-npc).
...
Decorative item displays. See [Floating Items](floating-items). These values are the
...
- [Modules](modules) — what each module enables.
- [Commands](commands) — `/hextras reload` and `/hextras module …`.
- [Visibility & Packets](visibility-and-packets) — the flags that gate packet features.
