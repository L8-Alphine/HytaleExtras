...
[`stringTemplate.placeholderApi.missingBehavior`](configuration#string-templates):
...
- [Effects](effects) and [Conditions](conditions) — which fields are templated.
- [Visibility & Packets](visibility-and-packets) — how `ViewerRule`/`TargetRule` drive hiding.
- [Configuration](configuration#string-templates) — enabling/disabling each resolution layer.
- [Developer API](developer-api#rules--placeholders) — `evaluateRule` and `resolveText`.
