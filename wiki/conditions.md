...
[`apply_cooldown`](effects#apply_cooldown).
...
evaluated by the [arithmetic engine](player-state#arithmetic).
...
### `interaction_type`
Filters interactions inside the [Interaction Bridge](interaction-bridge). Must be used on
...
Reads both static editor tags and runtime tags. See [`set_volume_tag`](effects#set_volume_tag).
...
[TagNPC effects](effects#tagnpc-entity-state). See [TagNPC](tag-npc).
...
- [Effects Reference](effects) — actions gated by these conditions.
- [String Templates](string-templates) — placeholders and the rule mini-language.
- [Native conditions](getting-started#native-vs-hyextras) — conditions already provided by Hytale (Permission, Cooldown, RandomChance, Tag, Item, BlockType, GameMode, PlayerCount).
