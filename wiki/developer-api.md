...
[internals docs](internals-services).
...
helpers return `false` for offline players. See [Player State](player-state).

---

## Variables

Per-player runtime values. See [Player State → Variables](player-state#variables).
...
Persistent boolean flags. See [Player State → Tags](player-state#tags).
...
Per-player runtime timers. See [Player State → Cooldowns](player-state#cooldowns).
...
override explicit hides — see [Visibility & Packets](visibility-and-packets).
...
[color/style codes](string-templates#color--style-codes) but does **not** resolve trigger-context
...
`attachmentId()` for attaches). See [Image Icons](image-icons).
...
See [Architecture](architecture) and [Internals: Services](internals-services) for how these
