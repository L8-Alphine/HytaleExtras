...
**Provided by:** the `image_icons` [module](modules). Driven entirely through the
[Developer API](developer-api#imageicons) — there are no trigger effects for it.
...
### Local hot reload
When [`imageIcons.hotReload=true`](configuration#imageicons), HyExtras watches each provider folder
...
size-limited by [`imageIcons.remoteCache.maxBytes`](configuration#imageicons) (default 5 MiB), then
...
capped by [`imageIcons.maxIconsPerViewer`](configuration#imageicons) (default 64).
...
- [Developer API → ImageIcons](developer-api#imageicons) — full method list.
- [Configuration](configuration#imageicons) — hot reload, remote cache, limits.
- [Modules](modules) — enabling/disabling `image_icons`.
