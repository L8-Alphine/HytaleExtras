...
[`hyextras.properties`](configuration#internal-modules).

## The six modules

| Module id | Provides |
|---|---|
| `trigger_extras` | All HyExtras trigger [effects](effects), [conditions](conditions), and the [interaction bridge](interaction-bridge). |
| `placeholder_api` | The PlaceholderAPI bridge for [string templates](string-templates#placeholderapi). Safe when PlaceholderAPI isn't installed. |
| `packet_api` | Packet-backed [visibility, camera, title](visibility-and-packets) services. |
| `image_icons` | Provider-scoped [image icons](image-icons). |
| `tag_npc` | [TagNPC](tag-npc) runtime entity state. |
| `floating_items` | Decorative [floating item](floating-items) displays. |

## Settings

Each module has three config keys (defaults in [Configuration](configuration#internal-modules)):
...
See [Commands → modules](commands#hextras-modules--hextras-module).

## See also

- [Configuration](configuration#internal-modules) — default settings.
- [Architecture](architecture) — how modules are registered at startup.
