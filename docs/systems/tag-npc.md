...
[player state](player-state.md) and is **runtime-only** (not persisted) in this version.

It's useful when a Trigger Volume context gives you a mob/NPC entity UUID, or when another mod calls
the [Developer API](../DEVELOPER_API.md#tagnpc) with an entity UUID. HyExtras also indexes UUID-backed
...
Every `tag_npc_*` [effect](../effects.md#tagnpc-entity-state) and
[condition](../conditions.md#tagnpc) shares a targeting trio:
...
[`tagNpc.clearStateOnEntityUnload=true`](../configuration.md#tagnpc), stale indexed entities (unseen for
...
per-viewer visibility by delegating to the [PacketAPI entity visibility](visibility-and-packets.md#non-player-entity-visibility)
...
Use [`/hextras tagnpc`](../commands.md#hextras-tagnpc) for explicit UUID edits, or the `near` subcommands
...
[Developer API → TagNPC](../DEVELOPER_API.md#tagnpc).
...
- [Effects](../effects.md#tagnpc-entity-state) · [Conditions](../conditions.md#tagnpc)
- [Visibility & Packets](visibility-and-packets.md) — the entity-filtering requirement.
- [Configuration](../configuration.md#tagnpc).
