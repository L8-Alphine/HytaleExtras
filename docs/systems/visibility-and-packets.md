...
1. **Server-side state** (`PlayerOverrideService`) — always updated. The
   [`player_hidden`](../conditions.md#player_hidden) condition tests this layer, so it works even with no
   packets sent.
2. **Client packet** (`HiddenPlayersManager`) — only sent when **both**:
   ...
   - `advancedPacketActions=true` and the `packet_api` module is enabled (see
     [Configuration](../configuration.md#global-flags)).
...
| Effect | Purpose |
|---|---|
| [`player_hide_entity`](../effects.md#player_hide_entity) | Hide a target player from the triggering viewer. |
| [`player_show_entity`](../effects.md#player_show_entity) | Reverse a hide. |
| [`clear_player_overrides`](../effects.md#clear_player_overrides) | Clear all overrides for the viewer and release targeting protection. |

Both hide/show support **rule-driven targeting** via `ViewerRule`/`TargetRule` and a `TargetSelector`
(`player`, `players`, `entities`). Rules use the [rule predicate](../string-templates.md#rule-predicates)
...
([Player State](player-state.md#variables)):
...
> [Developer API → Visibility](../DEVELOPER_API.md#visibility).

---

## Camera

[`set_camera`](../effects.md#set_camera) sends a `SetServerCamera` packet to **only** the triggering
...
[Developer API → Packets](../DEVELOPER_API.md#packets--messaging). All return `false` for offline players.
...
- It is released by `player_show_entity` with `PreventTargeting=true`,
  [`clear_player_overrides`](../effects.md#clear_player_overrides), disconnect, or shutdown.
...
timing. This path also backs [TagNPC](tag-npc.md) visibility. Raw packet sending is never exposed.

## See also

- [Configuration](../configuration.md#global-flags) — `advancedPacketActions`, `entityPacketFiltering`, `playerVisibilityPolicySync`.
- [Developer API](../DEVELOPER_API.md#visibility) — visibility, camera, targeting helpers.
- [Internals: Packet stack](../internals/packet-internals.md) — how the services are wired.
