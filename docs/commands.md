...
Manage per-player [variables](systems/player-state.md#variables) (runtime-only).
...
Manage persistent [player tags](systems/player-state.md#tags). All mutations persist immediately.
...
Inspect/clear named [cooldowns](systems/player-state.md#cooldowns).
...
Manage [internal modules](systems/modules.md). `module` actions respect the `allowInGameToggle` and
`reloadable` flags in [configuration](configuration.md#internal-modules).
...
Admin tools for [TagNPC](systems/tag-npc.md) runtime entity state. The `tag`/`var`/`hide`/`show`
...
Manage decorative [floating items](systems/floating-items.md). Each subcommand requires a permission
...
- [Configuration](configuration.md) — what `/hextras reload` and `/hextras module` change.
- [Player State](systems/player-state.md) — the data behind `var`, `tag`, and `cooldown`.
- [TagNPC](systems/tag-npc.md) and [Floating Items](systems/floating-items.md) — the systems behind those groups.
