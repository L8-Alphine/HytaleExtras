...
All live in `hyextras.properties` (see [Configuration](configuration.md) for the full list).
...
([`ComparisonOperator`](../src/main/java/org/hyzionstudios/hyextras/util/ComparisonOperator.java)):
...
Event types ([`HyExtrasEvents`](../src/main/java/org/hyzionstudios/hyextras/event/HyExtrasEvents.java)):
`PlayerTagChangeEvent`, `PlayerVariableChangeEvent`, `EntityTagChangeEvent`,
`EntityVariableChangeEvent`, `VisibilityChangeEvent`, `InteractionEvent`. Listeners run
synchronously on the posting thread and are isolated (one throwing listener can't break others).
...
the NPC/mob the player most recently interacted with.
...
"Locked" feedback. See [Interaction Bridge](systems/interaction-bridge.md).
...
- Batched hide/show packet flush; entity-type (NPC vs mob) classification in `EntityResolver`.
