...
Related concepts: [Player State](systems/player-state.md) · [Visibility & Packets](systems/visibility-and-packets.md) ·
[Interaction Bridge](systems/interaction-bridge.md) · [TagNPC](systems/tag-npc.md) ·
[Floating Items](systems/floating-items.md) · [Developer API](DEVELOPER_API.md).
...
[Player State](systems/player-state.md).
...
[arithmetic expression](systems/player-state.md#arithmetic) engine.
...
[Player State](systems/player-state.md#cooldowns) and the [`cooldown_ready`](conditions.md#cooldown_ready) condition.
...
Both command effects run as the triggering player and support [string templates](string-templates.md)
...
All messaging effects target the triggering player and support [color/style codes](string-templates.md#color--style-codes).
...
See [Visibility & Packets](systems/visibility-and-packets.md#camera) for camera behavior.
...
[Visibility & Packets](systems/visibility-and-packets.md) and the
[`player_hidden`](conditions.md#player_hidden) condition.
...
| `ViewerRule` | string *(rule)* | No | — | [Rule predicate](string-templates.md#rule-predicates) the viewer must match. |
...
[`volume_has_tag`](conditions.md#volume_has_tag) condition and
[Interaction Bridge](systems/interaction-bridge.md) for tag-event flow.
...
These work with the [Interaction Bridge](systems/interaction-bridge.md). `cancel_interaction` only has
...
directory). Distinct from variables. See [Player State](systems/player-state.md#tags) and the
[`has_tag`](conditions.md#has_tag) condition.
...
target-selection trio. See [TagNPC](systems/tag-npc.md).
...
Decorative, non-pickup item displays. See [Floating Items](systems/floating-items.md) and the
[`floating_item_exists`](conditions.md#floating_item_exists) /
[`floating_item_intangible`](conditions.md#floating_item_intangible) conditions.

### `floating_item_create`
Creates (or replaces) a floating item. Position comes from the `Anchor` plus optional explicit
coordinates and offsets; tuning fields fall back to [configuration](configuration.md#floatingitems)
...
- [Conditions Reference](conditions.md) — gate these effects.
- [String Templates](string-templates.md) — placeholders and color codes used in templated fields.
- [Commands](commands.md) — inspect/modify the same state from the console.
- [Developer API](DEVELOPER_API.md) — drive the same systems from Java.
- [What's native vs. HyExtras](getting-started.md#native-vs-hyextras) — effects already provided by Hytale.
