...
4. (Optional) Install [PlaceholderAPI for Hytale](string-templates.md#placeholderapi) to enable
...
`0.5.x` range. See [Architecture](architecture.md) for the project layout.
...
- **[Effects](effects.md)** *do* something (set a variable, send a title, hide a player…).
- **[Conditions](conditions.md)** *gate* effects — all conditions on a volume must pass or the effects
...
| `TAG_ADDED` | A tag is added to the volume (also used by the [interaction bridge](systems/interaction-bridge.md)). |
...
Field names are case-sensitive and match the [Effects](effects.md) / [Conditions](conditions.md)
reference exactly. Text fields support [String Templates](string-templates.md).
...
4. `send_title` greets the player, substituting the counter via a [placeholder](string-templates.md).

From here, explore the [Effects](effects.md) and [Conditions](conditions.md) reference, then the
[system deep-dives](README.md#systems) for visibility, the interaction bridge, TagNPC, and more.
...
items, image icons, and a developer API. See [What HyExtras adds](../README.md#at-a-glance).

## Next steps

- [Effects](effects.md) · [Conditions](conditions.md) · [Commands](commands.md) · [Configuration](configuration.md)
- [Documentation index](README.md) — every guide in one place.
- [Developer API](DEVELOPER_API.md) — drive HyExtras from another mod.
