...
> should use [`HyExtrasApi`](../DEVELOPER_API.md); these may change between releases. This page
...
[disconnect chain](../architecture.md#plugin-lifecycle).
...
Provider-scoped icon assets and runtime attachments. See [Image Icons](../systems/image-icons.md).
...
Registry and lifecycle for the six [modules](../systems/modules.md): `register`,
...
| Class | Role |
|---|---|
| `StringTemplate` | Resolves `{...}` template placeholders in trigger text. See [String Templates](../string-templates.md). |
...
| `HyExtrasConfig` / `ConfigLoader` | Config model and properties loader/migrator. See [Configuration](../configuration.md). |
| `CodecHelper` | `KeyedCodec` factory helpers (`string`, `optString`, `optEnum`, …) used by every effect/condition. |

## See also

- [Architecture](../architecture.md) — how these are wired together.
- [Internals: TriggerExtras](trigger-extras.md) · [Internals: Packet stack](packet-internals.md)
- [Developer API](../DEVELOPER_API.md) — the stable surface that wraps these.
