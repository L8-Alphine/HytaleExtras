...
> [Trigger Volume Research](../TRIGGER_VOLUME_RESEARCH.md).
...
[`CodecHelper`](services.md#supporting-utilities-util-config-codec):
...
**synthetic** events — interaction-bridge dispatch and [`trigger_named_volume`](../effects.md#trigger_named_volume)
chaining. `dispatch(...)`:
...
See [Interaction Bridge](../systems/interaction-bridge.md) for the creator-facing behavior.
...
3. Implement `execute(ctx)` / `test(ctx)`. Guard with `TriggerExtrasRuntime.isEnabled()` (+ the owning
   ...
   text via [`StringTemplate.resolve`](../string-templates.md). Wrap the body in try/catch and log on
...
4. Register it: add the type id to `TYPE_IDS` and a `registerEffect`/`registerCondition` line in the
   matching registry.
5. Document it in [`docs/effects.md`](../effects.md) or [`docs/conditions.md`](../conditions.md) and add
   an example to `examples/hyextras-examples.json`.

## See also

- [Architecture](../architecture.md#registration--data-flow) · [Internals: Services](services.md) · [Internals: Packet stack](packet-internals.md)
- [Effects](../effects.md) · [Conditions](../conditions.md)
