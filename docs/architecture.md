...
[Internals: Services](internals/services.md), [Internals: TriggerExtras](internals/trigger-extras.md),
and [Internals: Packet stack](internals/packet-internals.md). Notes on the **native** Hytale API live
in [Trigger Volume Research](TRIGGER_VOLUME_RESEARCH.md).
...
- **[`HyExtrasApi`](DEVELOPER_API.md)** is the only stable surface for other mods. It delegates to the
  plugin's services.
...
- **[Modules](systems/modules.md)** gate whole feature areas on/off; most effects/conditions re-check
...
**`start()`** — loads [config](configuration.md), constructs every service, builds the
...
- `PlayerInteractEvent` (global) → the [interaction bridge](systems/interaction-bridge.md).
...
- [`TriggerActionRegistry.registerAll()`](internals/trigger-extras.md) registers all 41 effect type
...
| `TargetingPreventionSystem` | Each tick, clears protected players from nearby NPC `TargetMemory` (see [targeting protection](systems/visibility-and-packets.md#targeting-protection)). |
| `TagNpcEntityIndexSystem` | Indexes UUID-backed entities with positions so [`/hextras tagnpc near`](commands.md#hextras-tagnpc) and `findNearestTagNpcEntity` work. |

## See also

- [Internals: Services](internals/services.md) · [Internals: TriggerExtras](internals/trigger-extras.md) · [Internals: Packet stack](internals/packet-internals.md)
- [Modules](systems/modules.md) — the runtime gating layer.
- [Trigger Volume Research](TRIGGER_VOLUME_RESEARCH.md) — native API notes.
